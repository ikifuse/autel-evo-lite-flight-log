...
**00:00は管理開始時の初期値であり、毎回00:00へ戻す意味ではありません。** 原則は製造後の積算時間です。従来記録がない場合、国交省ガイドラインには新制度運用開始以降から起算し、その旨を日誌冒頭等へ明記する扱いがあります。任意のアプリ導入日で過去累計をリセットする意味ではありません。新品Liteと中古Lite+の[起算根拠・確認範囲](./docs/design/07-maintenance-start.md#機体別の累計起算根拠)は個別に整理しています。上のWebアプリ画面紹介とは別のサンプル運航です。
...
**正式記録は、アプリ・帳票・点検整備記録・必要時の手動補記を合わせて管理します。** 整備は実施時に原本を複製して記入し、予定を実績として記録しません。条件付き事項や機体別の累計起算根拠は[正式な運用方針](./docs/design/02-legal-operation.md)を参照してください。
...
開発者向けのビルド、分割構成、保存の冪等性、テスト、障害復旧などの詳細や各文書の役割は、[ドキュメント案内](./docs/index.md)、[設計書目次（担当章だけ読む）](./01_ドローン運航記録_設計書.md)、[コードマップ](./docs/code-map.md)を参照してください。
