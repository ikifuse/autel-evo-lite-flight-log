...
左のNo.1枠は **EVO Lite・20分**、右のNo.2枠は **EVO Lite+・20分**。日常点検と各飛行を、同じ帳票の2枠へ記録する例です。上のWebアプリ画面紹介とは別のサンプル運航です。
...
機種ごとの「点検時の総飛行時間」はそれぞれ **00:20**。この点検整備記録は日常点検とは別の帳票です。定期点検・整備の結果欄は運航保存で自動的に埋まるものではないため、未記入のまま掲載しています。
