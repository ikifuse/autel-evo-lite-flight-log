...
<p align="center">
Autel EVO Lite / Lite+ 向けに作成した、スマートフォン操作対応のドローン運航記録Webアプリです。<br>
飛行前点検から着陸・飛行後点検までを入力し、最後にGoogleスプレッドシートへ一括保存します。
</p>

<p align="center">
  <img src="./icon.png" width="120" height="120" alt="濃紺背景に白いドローンとオレンジのチェックを配した専用アイコン">
</p>


## このアプリを作った理由

**このアプリは、特定飛行で必要となる飛行記録・日常点検記録・点検整備記録を、紙だけに頼らず、現場でスマートフォンから確認・入力・保存できるようにすることを目的として作成しました。特定飛行以外についても、安全管理のため同様の記録を残せる運用を想定しています。**

**Googleスプレッドシートは単なるデータ保存先ではなく、必要な記録内容を人が直接確認・補記し、必要に応じて印刷できる「記録帳」として利用しています。**

国土交通省の「無人航空機の飛行日誌の取扱要領」では、必要な記載内容を網羅することを前提に電磁的な記録・保管が認められており、飛行時には紙媒体または電磁的記録で参照・提示可能な状態にしておくこととされています。本アプリは、この考え方を踏まえて、現場でスマートフォン1台から記録を扱いやすくすることを目指しています。

 **目指しているのは、紙の帳票を何枚も持ち歩くのではなく、スマホとドローン一式を持って、必要な確認と記録をその場で済ませながら気軽に飛ばしに行ける運用です。**
...
**00:00は管理開始時の初期値であり、毎回00:00へ戻す意味ではありません。** 原則は製造後の積算時間です。従来記録がない場合、国交省ガイドラインには新制度運用開始以降から起算し、その旨を日誌冒頭等へ明記する扱いがあります。任意のアプリ導入日で過去累計をリセットする意味ではありません。新品Liteと中古Lite+の[起算根拠・確認範囲](01_ドローン運航記録_設計書/07_点検整備・管理開始.md#機体別の累計起算根拠)は個別に整理しています。上のWebアプリ画面紹介とは別のサンプル運航です。
...
**正式記録は、アプリ・帳票・点検整備記録・必要時の手動補記を合わせて管理します。** 整備は実施時に原本を複製して記入し、予定を実績として記録しません。条件付き事項や機体別の累計起算根拠は[正式な運用方針](01_ドローン運航記録_設計書/02_法令・運用.md)を参照してください。
...
開発者向けのビルド、分割構成、保存の冪等性、テスト、障害復旧などの詳細や各文書の役割は、[ドキュメント案内](./docs/index.md)、[設計書目次（担当章だけ読む）](01_ドローン運航記録_設計書/00_目次.md)、[コードマップ](./docs/code-map.md)を参照してください。

## 公開範囲とライセンス

このリポジトリは、実装と改善履歴を確認できるよう公開されています。

本リポジトリは [MIT License](./LICENSE) のもとで公開されています。利用・改変・再配布などの条件については、リポジトリ内の `LICENSE` ファイルを参照してください。
