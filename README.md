...
**00:00は管理開始時の初期値であり、毎回00:00へ戻す意味ではありません。** 原則は製造後の積算時間です。従来記録がない場合、国交省ガイドラインには新制度運用開始以降から起算し、その旨を日誌冒頭等へ明記する扱いがあります。任意のアプリ導入日で過去累計をリセットする意味ではありません。新品Liteと中古Lite+の[起算根拠・確認範囲](./01_ドローン運航記録_設計書.md#23-機体別の累計起算根拠)は個別に整理しています。上のWebアプリ画面紹介とは別のサンプル運航です。
...
**正式記録は、アプリ・帳票・点検整備記録・必要時の手動補記を合わせて管理します。** 整備は実施時に原本を複製して記入し、予定を実績として記録しません。条件付き事項や機体別の累計起算根拠は[正式な運用方針](./01_ドローン運航記録_設計書.md#21-正式記録全体で評価する決定済みの役割分担)を参照してください。
